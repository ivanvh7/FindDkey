...
// Equivalencias por categorías
$equivalencias = [
    'keyboard' => ['teclado', 'teclados', 'keyboard', 'keyboards'],
    'switch' => ['switch', 'switches'],
    'keycap' => ['keycap', 'keycaps', 'tecla', 'teclas'],
    'stabilizer' => ['estabilizador', 'estabilizadores', 'stabilizer'],
    'silenciador' => ['silenciador', 'silenciadores', 'foam'],
    'pcb' => ['pcb', 'placa', 'placa base'],
    'deskmat' => ['deskmat', 'deskmats', 'alfombrilla', 'alfombrillas'],
    'tool' => ['tool', 'tools', 'herramienta', 'herramientas', 'toolkit'],
    'lube' => ['lube', 'lubricante', 'lubricantes'],
];

// Obtener todas las keywords relacionadas con la búsqueda
$busqueda_keywords = [$q];
foreach ($equivalencias as $grupo) {
    if (in_array($q, $grupo)) {
        $busqueda_keywords = $grupo;
        break;
    }
}
...
foreach ($fuentes as $tabla => $productos) {
    foreach ($productos as $producto) {
        $nombre = strtolower($producto['nombre'] ?? '');
        $descripcion = strtolower($producto['descripcion'] ?? '');

        foreach ($busqueda_keywords as $palabra) {
            if (str_contains($nombre, $palabra) || str_contains($descripcion, $palabra)) {
                $producto['tabla'] = $tabla; // Añadir tabla sin tocar equivalencias
                $resultados[] = $producto;
                break;
            }
        }
...
<main style="margin-top:60px;">
  <h1 style="text-align:left;">Busqueda</h1>
  <nav class="breadcrumb">
    <a href="index.php">Home</a> / <span>Sales</span>
  </nav>

<?php if (empty($resultados)): ?>
  <p style="text-align:center; color:red;">No se encontraron productos.</p>
<?php else: ?>
  <div class="products">
    <?php foreach ($resultados as $p): ?>
      <a href="detalle_producto.php?tabla=<?= urlencode($p['tabla']) ?>&id=<?= urlencode($p['id']) ?>" class="product-card" style="text-decoration: none; color: inherit;">
