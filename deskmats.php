...
$sql = "SELECT * FROM deskmats WHERE 1=1";
$params = [];
$types = "";
...
<h1 style="text-align:left;">Deskmats</h1>
  <nav class="breadcrumb">
    <a href="index.php">Home</a> / <span>Deskmats</span>
  </nav>

  <form method="get" action="deskmats.php" class="filtro-form">
    <label for="tamanio">Tamaño:</label>
    <select name="tamanio" id="tamanio">
      <option value="">Todos</option>
      <option value="900x400" <?php if ($tamanio == '900x400') echo 'selected'; ?>>900x400</option>
      <option value="800x300" <?php if ($tamanio == '800x300') echo 'selected'; ?>>800x300</option>
      <option value="XL" <?php if ($tamanio == 'XL') echo 'selected'; ?>>XL</option>
      <option value="M" <?php if ($tamanio == 'M') echo 'selected'; ?>>M</option>
      <option value="Custom" <?php if ($tamanio == 'Custom') echo 'selected'; ?>>Custom</option>
    </select>
  </form>

<div class="products">
  <?php while ($row = $result->fetch_assoc()): ?>
    <a href="detalle_producto.php?tabla=deskmats&id=<?php echo urlencode($row['id']); ?>" class="product-card" style="text-decoration: none; color: inherit;">
      ...
      <p>
        <?php if (!empty($row['precio_original']) && $row['precio_original'] > $row['precio']): ?>
          <span style="text-decoration: line-through; color: #888;"><?php echo number_format($row['precio_original'], 2); ?> €</span>
        <?php endif; ?>
...
<section class="highlight-section">
  ...
  <div class="highlight-content">
    ...
    <a href="keycaps.php" class="highlight-btn">Shop Now</a>
  </div>
</section>
