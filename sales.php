...
function obtenerProductosConDescuento($tabla) {
    global $conn;
    $permitidas = ['keyboards', 'switches', 'keycaps', 'stabilizers', 'silenciadores', 'pcb', 'deskmats', 'tools', 'lube'];
    if (!in_array($tabla, $permitidas)) return [];

    $sql = "SELECT *, '$tabla' AS origen FROM $tabla WHERE precio_original IS NOT NULL AND precio_original > precio";
    $res = $conn->query($sql);
    $datos = [];
...
$productos = array_merge(
    obtenerProductosConDescuento('keyboards'),
    obtenerProductosConDescuento('switches'),
    obtenerProductosConDescuento('keycaps'),
    obtenerProductosConDescuento('stabilizers'),
    obtenerProductosConDescuento('silenciadores'),
    obtenerProductosConDescuento('pcb'),
    obtenerProductosConDescuento('tools'),
    obtenerProductosConDescuento('lube'),
    obtenerProductosConDescuento('deskmats')
);
?>
    ...
    <?php if (count($productos) > 0): ?>
      <?php foreach ($productos as $p): ?>
        <div class="product-card">
          ...
          <p>
            <span style="text-decoration: line-through; color: #888;"><?php echo $p['precio_original']; ?> €</span>
            <strong style="color: red;"><?php echo $p['precio']; ?> €</strong>
          </p>
        </div>
      <?php endforeach; ?>
    <?php else: ?>
      <p style="text-align:center;">No hay productos en oferta actualmente.</p>
    <?php endif; ?>
  </div>
</main>
