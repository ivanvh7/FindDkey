...
<main style="margin-top:60px;">
    <h1 style="text-align:left;">Mechanical keyboards</h1>
    <nav class="breadcrumb">
        <a href="index.php">Home</a> / <span>Keyboards</span>
    </nav>

    <form method="get" action="teclados.php" class="filtro-form">
        <label for="marca">Marca:</label>
        <select name="marca" id="marca">
            <option value="">Todas</option>
            <option value="Keychron" <?php if ($marca == 'Keychron') echo 'selected'; ?>>Keychron</option>
            <option value="QwertyKeys" <?php if ($marca == 'QwertyKeys') echo 'selected'; ?>>QwertyKeys</option>
            <option value="Geon" <?php if ($marca == 'Geon') echo 'selected'; ?>>Geon</option>
            <option value="Eloquent" <?php if ($marca == 'Eloquent') echo 'selected'; ?>>Eloquent</option>
            <option value="Nuphy" <?php if ($marca == 'Nuphy') echo 'selected'; ?>>Nuphy</option>
        </select>

        <label for="tamanio">Tamaño:</label>
        <select name="tamanio" id="tamanio">
            <option value="">Todos</option>
            <option value="Full-size" <?php if ($tamanio == 'Full-size') echo 'selected'; ?>>Full-size</option>
            <option value="TKL" <?php if ($tamanio == 'TKL') echo 'selected'; ?>>TKL</option>
            <option value="75%" <?php if ($tamanio == '75%') echo 'selected'; ?>>75%</option>
            <option value="65%" <?php if ($tamanio == '65%') echo 'selected'; ?>>65%</option>
            <option value="60%" <?php if ($tamanio == '60%') echo 'selected'; ?>>60%</option>
            <option value="40%" <?php if ($tamanio == '40%') echo 'selected'; ?>>40%</option>
        </select>

        <label for="lenguaje">Lenguaje:</label>
        <select name="lenguaje" id="lenguaje">
            <option value="">Todos</option>
            <option value="ANSI" <?php if ($lenguaje == 'ANSI') echo 'selected'; ?>>ANSI (English)</option>
            <option value="ISO" <?php if ($lenguaje == 'ISO') echo 'selected'; ?>>ISO (Spanish)</option>
        </select>

        <label for="features">Features:</label>
        <select name="features" id="features">
            <option value="">Todos</option>
            <option value="Wireless" <?php if ($features == 'Wireless') echo 'selected'; ?>>Wireless</option>
            <option value="Hot-swap" <?php if ($features == 'Hot-swap') echo 'selected'; ?>>Hot-swap</option>
            <option value="RGB" <?php if ($features == 'RGB') echo 'selected'; ?>>RGB</option>
            <option value="Low-profile" <?php if ($features == 'Low-profile') echo 'selected'; ?>>Low-profile</option>
            <option value="Gasket mount" <?php if ($features == 'Gasket mount') echo 'selected'; ?>>Gasket Mount</option>
        </select>
    </form>

<div class="products">
    <?php while ($row = $result->fetch_assoc()): ?>
        <a href="detalle_producto.php?tabla=keyboards&id=<?php echo urlencode($row['id']); ?>" class="product-card" style="text-decoration: none; color: inherit;">
            <img src="<?php echo $row['imagen']; ?>" alt="<?php echo $row['nombre']; ?>">
            <h2><?php echo $row['nombre']; ?></h2>
            <p><?php echo $row['descripcion']; ?></p>
            <p>
                <?php if (!empty($row['precio_original']) && $row['precio_original'] > $row['precio']): ?>
