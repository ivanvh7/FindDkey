...
<main style="margin-top:60px;">
  <h1 style="text-align:left;">Switches</h1>
  <nav class="breadcrumb">
    <a href="index.php">Home</a> / <span>Switches</span>
  </nav>

  <form method="get" action="switches.php" class="filtro-form">
    <label for="tipo">Tipo:</label>
    <select name="tipo" id="tipo">
      <option value="">Todos</option>
      <option value="Lineal" <?php if ($tipo == 'Lineal') echo 'selected'; ?>>Lineal</option>
      <option value="Táctil" <?php if ($tipo == 'Táctil') echo 'selected'; ?>>Táctil</option>
      <option value="Clicky" <?php if ($tipo == 'Clicky') echo 'selected'; ?>>Clicky</option>
    </select>
  </form>

<div class="products">
  <?php while ($row = $result->fetch_assoc()): ?>
    <a href="detalle_producto.php?tabla=switches&id=<?php echo urlencode($row['id']); ?>" class="product-card" style="text-decoration: none; color: inherit;">
